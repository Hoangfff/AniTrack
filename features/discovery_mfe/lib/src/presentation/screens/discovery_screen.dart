...
class DiscoveryScreen extends ConsumerWidget {
      ...
      body: CustomScrollView(
        slivers: [
          SliverAppBar(
            ...
            title: Text(
              'Discovery',
              style: AniTrackTypography.displayMedium.copyWith(color: AniTrackColors.onBackground),
            ),
            floating: true,
          ),
          SliverToBoxAdapter(
  ...
  Widget _buildSectionHeader(String title) {
    return Padding(
      ...
      child: Text(
        title,
        style: AniTrackTypography.headlineLarge.copyWith(color: AniTrackColors.onBackground),
      ),
    );
  }
  ...
  Widget _buildSeasonalAnime(WidgetRef ref) {
    ...
    return _buildAsyncRow(seasonalAsync, onRetry: () => ref.invalidate(seasonalAnimeProvider));
  }

  Widget _buildTopAnime(WidgetRef ref) {
    final topAsync = ref.watch(topAnimeProvider);
    return _buildAsyncRow(topAsync, onRetry: () => ref.invalidate(topAnimeProvider));
  }
  ...
  Widget _buildAnimeByGenre(WidgetRef ref) {
    ...
    return genresAsync.when(
      data: (genres) {
        ...
        final targetGenre = genres.firstWhere((g) => g.id == targetGenreId, orElse: () => genres.first);
        
        final animeByGenreAsync = ref.watch(animeByGenreProvider(targetGenre.id));
        
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: AniTrackSpacing.lg, vertical: AniTrackSpacing.xs),
                ...
                child: Wrap(
                  ...
                  children: genres.map((genre) {
                    ...
                    return ChoiceChip(
                      ...
                      onSelected: (selected) {
                        if (selected) {
                          ref.read(selectedGenreIdProvider.notifier).state = genre.id;
                        }
                      },
                      selectedColor: AniTrackColors.primaryContainer,
                      backgroundColor: AniTrackColors.surfaceVariant,
                      labelStyle: TextStyle(
                        color: isSelected ? AniTrackColors.primary : AniTrackColors.onSurface,
                        fontWeight: isSelected ? FontWeight.bold : FontWeight.normal,
                      ),
                      shape: RoundedRectangleBorder(
                        borderRadius: AniTrackSpacing.radiusSmall,
                        side: BorderSide(
                          color: isSelected ? AniTrackColors.primary : Colors.transparent,
                        ),
                      ),
                    );
                  }).toList(),
            ...
            _buildAsyncRow(animeByGenreAsync, onRetry: () => ref.invalidate(animeByGenreProvider(targetGenre.id))),
          ],
        );
      },
  ...
  Widget _buildAsyncRow(AsyncValue<List<AnimeModel>> asyncValue, {VoidCallback? onRetry}) {
